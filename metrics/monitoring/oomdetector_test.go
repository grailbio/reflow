...
func TestNodeOomDetector(t *testing.T) {
	m := &oomDetector{timeSeries: newTimeSeries("", time.Millisecond,
		[]string{pressureMemWaiting, pressureMemStalled, memAvailableBytes, memTotalBytes}, nil)}
	m.setupConsumerFn()
	start := time.Now()
	...
	f := &testFetcher{start: start, p: 10 * time.Second, mType: dto.MetricType_GAUGE,
		values: []map[string]float64{
			{pressureMemStalled: 0.0, memAvailableBytes: 80 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 0.0, memAvailableBytes: 20 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 0.0, memAvailableBytes: 2 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 0.0, memAvailableBytes: 3 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 0.0, memAvailableBytes: 4 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 0.0, memAvailableBytes: 1 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 0.0, memAvailableBytes: 31 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 1.0, memAvailableBytes: 32 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 1.0, memAvailableBytes: 33 * memScale, memTotalBytes: memTotal},
			{pressureMemStalled: 1.0, memAvailableBytes: 34 * memScale, memTotalBytes: memTotal},
		},
	}
	m.fetcherFn = f.fetch
	...
	for i, tt := range []struct {
		...
		// Tests for pressureMemStalled-based oom determination
		{start.Add(time.Second), start.Add(19 * time.Second), false, ""},
		{start.Add(55 * time.Second), start.Add(75 * time.Second), true, "change in node_pressure_memory_stalled_seconds_total: 1.0000"},
		{start.Add(59 * time.Second), start.Add(99 * time.Second), true, "change in node_pressure_memory_stalled_seconds_total: 1.0000"},
