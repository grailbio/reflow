...
func TestScanTemplate(t *testing.T) {
	s := new(Scanner).Init(strings.NewReader(`foo {" bar baz "} bar`))
	checkTok(t, s, 1, s.Scan(), Ident, "foo")
	checkTok(t, s, 1, s.Scan(), Template, `{" bar baz "}`)
	checkTok(t, s, 1, s.Scan(), Ident, "bar")
