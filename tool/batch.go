...
func (s inlineOrFileSystemSourcer) Source(path string) (p []byte, d digest.Digest, err error) {
	var ok bool
	if p, ok = s[path]; !ok {
		p, d, err = syntax.Filesystem.Source(path)
		if err != nil {
			err = fmt.Errorf("module %s: %v", path, err)
		}
	}
	return
}
